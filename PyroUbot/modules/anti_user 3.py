...
async def add_user_to_blacklist(c, m):
    ...
    # Mendapatkan user ID berdasarkan reply atau command
    if m.reply_to_message:
        user_id = m.reply_to_message.from_user.id
    else:
        user_id = int(m.command[1])

    # Cek jika ID pengguna adalah pemilik bot, jika iya, maka tidak bisa ditambahkan
    if user_id == OWNER_ID:
        return await m.reply(f"<blockquote>{ggl}<b>ɢᴜᴀ ʏᴀɴɢ ᴘᴜɴʏᴀ ʙᴏᴛ ᴛᴏᴅ!</b></blockquote>")
    
    # Mendapatkan daftar blacklist pribadi pengguna bot
    user_ids = await get_user_ids(c.me.id)
    if user_id not in user_ids:
        user_ids.append(user_id)
        await user_collection.update_one({"_id": c.me.id}, {"$set": {"user_dia": user_ids}}, upsert=True)
...
# Menghapus pengguna dari daftar blacklist berdasarkan chat pribadi
@PY.UBOT("yespc")
@PY.ULTRA
async def remove_user_from_blacklist(c, m):
    brhsl = await EMO.BERHASIL(c)
    ...
    # Mendapatkan daftar pengguna yang ada dalam blacklist pribadi bot
    user_ids = await get_user_ids(c.me.id)
    if user_id in user_ids:
        user_ids.remove(user_id)
        await user_collection.update_one({"_id": c.me.id}, {"$set": {"user_dia": user_ids}}, upsert=True)
